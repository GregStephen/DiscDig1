...
class AddAlbumPagination extends React.Component {
  ...
  createSmallerThan10Pagination = () => {
    ...
    const pagesToReturn = arrayOfPages.map((page, i) => <PaginationItem active={i + 1 === currentPage} key={i + 1}>
  ...
  createLargerThan10Pagination = () => {
    const { currentPage, totalPages } = this.props;
    const arrayOfPages = [...Array(10)];
    let pagesToReturn;
    // if the current page is less than 6, then you will have an 'etc' item pushed at the end after 10
    if (currentPage < 6) {
      pagesToReturn = arrayOfPages.map((page, i) => <PaginationItem active={i + 1 === currentPage} key={i + 1}>
        <PaginationLink onClick={(e) => this.handleClick(e, i + 1)} href="#">
          {i + 1}
        </PaginationLink>
      </PaginationItem>);
    ...
    // than you would have the 'etc' item at the front and the end
      ...
      pagesToReturn = arrayOfPages.map((page, i) => <PaginationItem active={(i + 1 + (currentPage - 5)) === currentPage} key={(i + 1 + (currentPage - 5))}>
    ...
    // only have an 'etc' item at the beginnint
      ...
      pagesToReturn = arrayOfPages.map((page, i) => <PaginationItem active={(i + 1 + (totalPages - 10)) === currentPage} key={(i + 1 + (totalPages - 10))}>
